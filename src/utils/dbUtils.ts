...
export const insertPlayer = async (db: Promise<Database>, player: Player): Promise<number> => {
  try {
    ...
    const insertResult = await database.execute(query, [
      player.class_name,
      player.name,
      player.level,
      player.class_name,
...
export const fetchPlayerResistances = async (db: Promise<Database>, playerId: number): Promise<string> => {
  try {
    ...
    return database.select(
      'SELECT player_id,fire_resistance,magic_resistance,frost_resistance,lightning_resistance FROM player_stats WHERE player_id = $1',
      [playerId],
    );
  } catch (error) {
...
export const getClassDescription = async (db: Promise<Database>, className: string): Promise<string> => {
  try {
    const database = await db;
    const classData: { description: string }[] = await database.select(
      'SELECT description FROM classes WHERE name = $1',
      [className],
    );
